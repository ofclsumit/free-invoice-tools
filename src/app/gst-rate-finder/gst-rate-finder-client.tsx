...
import {
  ...
  UltraHeader, UltraCard, UltraCardHeader, UltraSectionLabel, UltraPrimaryButton,
      ...
      <UltraShell>
        ...
        <UltraPage>
          ...
          <UltraGrid>
            {/* ─── SEARCH CARD ─── */}
            <UltraCard>
              <UltraCardHeader title="Search GST Rates" />

              <div className="mb-4">
                <UltraSectionLabel>Search HSN or Category</UltraSectionLabel>
                <div className="flex items-center bg-white dark:bg-black/30 border border-slate-300 dark:border-white/[.12] rounded-[.75rem] overflow-hidden transition-all duration-200 focus-within:border-violet-600 focus-within:ring-2 focus-within:ring-violet-500/20 shadow-xs">
                  <span className="px-[.75rem] py-[.7rem] text-slate-500 dark:text-[#a78bfa]/70 border-r border-slate-200 dark:border-white/[.08] shrink-0 bg-slate-50 dark:bg-transparent">
                  ...
                  <input
                    ...
                    className="flex-1 min-w-0 px-[.9rem] py-[.7rem] bg-transparent border-none text-slate-900 dark:text-white text-[.95rem] font-medium font-sans outline-none placeholder:text-slate-400 dark:placeholder-white/30"
                  />
                </div>
              </div>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
                      selectedCategory === cat
                        ? "bg-violet-600 text-white shadow-xs dark:bg-gradient-to-r dark:from-indigo-500 dark:to-purple-500"
                        : "bg-slate-100 border border-slate-200 text-slate-700 hover:bg-slate-200 dark:bg-black/40 dark:border-white/[0.12] dark:text-white/70 dark:hover:text-white"
                    }`}
                  >
                    {cat}
                  </button>
            ...
            <UltraCard>
              <div className="flex items-center justify-between mb-[1.35rem]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[1rem] font-bold text-slate-900 dark:text-white">Results</span>
                </div>
                <p className="text-xs font-semibold text-slate-600 dark:text-white/60">{filtered.length} result{filtered.length !== 1 ? "s" : ""}</p>
              </div>

              <div className="bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.12] rounded-xl overflow-hidden shadow-xs">
                <div className="grid grid-cols-[1fr_80px_60px_60px_60px] sm:grid-cols-[120px_1fr_70px_60px_60px_60px] gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 text-xs font-bold text-slate-700 dark:text-white/70 uppercase tracking-wider border-b border-slate-200 dark:border-white/[0.12] bg-slate-50 dark:bg-black/40">
                ...
                <div className="divide-y divide-slate-100 dark:divide-white/[0.06] max-h-[500px] overflow-y-auto">
                  {filtered.map((d, idx) => (
                    <div key={idx} className="grid grid-cols-[1fr_80px_60px_60px_60px] sm:grid-cols-[120px_1fr_70px_60px_60px_60px] gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 text-sm hover:bg-slate-50 dark:hover:bg-white/[0.04] transition-colors items-center">
                      <span className="text-xs font-semibold text-slate-700 dark:text-white/70 truncate">{d.category}</span>
                      <span className="text-slate-800 dark:text-white/80 truncate text-xs sm:text-sm font-medium" title={d.description}>{d.description}</span>
                      <span className="font-mono text-xs font-bold text-violet-700 dark:text-indigo-400 truncate">{d.hsn}</span>
                      <span className={`font-bold font-mono ${d.rate === 0 ? "text-emerald-700 dark:text-emerald-400" : d.rate >= 28 ? "text-rose-700 dark:text-rose-400" : "text-amber-700 dark:text-amber-400"}`}>{d.rate}%</span>
                      <span className="text-xs font-mono text-slate-600 dark:text-white/65">{d.cgst}%</span>
                      <span className="text-xs font-mono text-slate-600 dark:text-white/65">{d.sgst}%</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {filtered.length === 0 && (
                <p className="text-center text-sm font-medium text-slate-500 dark:text-white/60 py-8">No GST rates found for &quot;{search}&quot;</p>
              )}
              ...
              <div className="flex justify-center mt-5">
                <UltraPrimaryButton onClick={handleDownloadPDF} disabled={isDownloading || filtered.length === 0} className="!px-4 !py-2 !text-xs !rounded-lg">
                  <Download className="h-3.5 w-3.5" /> {isDownloading ? "Exporting..." : "Export PDF"}
                </UltraPrimaryButton>
              </div>

              <p className="text-[11px] text-slate-500 dark:text-white/50 text-center mt-4">GST rates are indicative. Please refer to official GST portal for statutory rates.</p>
            </UltraCard>
          </UltraGrid>
      </UltraPage>
    </UltraShell>
