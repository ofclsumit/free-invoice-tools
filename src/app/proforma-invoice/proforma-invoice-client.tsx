...
export function ProformaInvoiceClient() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      if (node) {
        await exportNodeToPdf(node, `proforma-${watchedValues.proformaNumber}.pdf`);
        toast({ title: "Proforma Invoice PDF downloaded!" })
      }
    } catch {
  ...
  return (
    <>
      {isGenerating && <LoadingScreen message="Generating Proforma PDF..." />}
          ...
          <div className="relative w-full h-full flex flex-col max-w-[1200px] mx-auto bg-white/5 dark:bg-black/5 shadow-2xl animate-in slide-in-from-bottom-8 zoom-in-95 duration-500 overflow-hidden">
            ...
            <div className="flex items-center justify-between p-4 border-b border-white/10 bg-white dark:bg-gray-950 sticky top-0 z-10 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <h2 className="text-lg font-display font-semibold hidden sm:block">Proforma Preview</h2>
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={handleSaveProforma} className="hidden sm:flex gap-2">
                  <Save className="h-4 w-4" /> Save
                </Button>
                <Button size="sm" className="gap-2 bg-gradient-to-r from-teal-600 to-emerald-600 text-white border-0 font-semibold" onClick={handleDownloadPDF} disabled={isGenerating}>
        ...
        <div className="max-w-4xl mx-auto w-full">
          <div className="space-y-6 min-w-0">
            ...
            <div className="flex flex-wrap gap-3 pb-4">
              <Button className="gap-2 bg-gradient-to-r from-teal-600 to-emerald-600 text-white border-0 font-semibold" onClick={togglePreview}>
              ...
              <button onClick={handleEmailProforma} title="Email Proforma" className="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-input bg-background shadow-sm transition-colors hover:bg-accent cursor-pointer">
                <img src="/email.svg" alt="Email" className="h-5 w-5" />
              </button>
              <ShareButton invoiceData={invoiceData} template={watchedValues.template} title={`Proforma ${watchedValues.proformaNumber}`} />
            ...
            <section className="form-section overflow-hidden">
              ...
              <p className="text-[11px] text-muted-foreground mt-2 flex items-center gap-1">
                ...
                This is how your proforma will look. Data updates in real-time.
              </p>
            </section>
