...
const handleDownloadPDF = async () => {
    ...
    try {
      ...
      if (node) {
        await exportNodeToPdf(node, `credit-note-${creditNoteNo || "draft"}.pdf`);
      }
    } finally {
          ...
          <div className="max-w-4xl mx-auto w-full flex flex-col h-full">
            <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
              <Button variant="outline" onClick={handleDownloadPDF} disabled={isGenerating} className="gap-2 bg-white">
                <ArrowLeft className="h-4 w-4" /> Edit Credit Note
              </Button>
              <Button onClick={handleDownloadPDF} disabled={isGenerating} className="gap-2 bg-gradient-to-r from-amber-500 to-yellow-600 text-white border-0 shadow-lg hover:shadow-xl transition-all">
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          ...
          <Input placeholder="Auto-generated" value={creditNoteNo} onChange={e => setCreditNoteNo(e.target.value)} className="h-9 text-sm" />
        </div>
        <div className="space-y-1.5">
          <Label className="text-xs font-medium">Date</Label>
          <Input type="date" className="h-9 text-sm" value={creditDate} onChange={e => setCreditDate(e.target.value)} />
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        <Label className="text-xs font-medium">Reason for Credit</Label>
        <Textarea placeholder="Why is this credit note being issued?" value={reason} onChange={e => setReason(e.target.value)} className="text-sm resize-none" rows={2} />
      </div>
      ...
      <div className="space-y-2 sm:ml-auto sm:w-60">
        ...
        <div className="flex justify-between font-display font-bold text-lg">
          ...
          <span className="text-amber-600">₹{fmt(totals.grandTotal)}</span>
        </div>
      </div>

      <Button onClick={handleDownloadPDF} disabled={isGenerating} className="w-full bg-gradient-to-r from-amber-500 to-yellow-600 text-white border-0 font-semibold gap-2">
