...
export function CreditNoteClient() {
  ...
  const handleDownloadPDF = async (returnBlob?: boolean): Promise<Blob | void> => {
    ...
    try {
      const node = document.getElementById("credit-note-print-root")
      if (node) {
        const blob = await exportNodeToPdf(node, `credit-note-${creditNoteNo || "draft"}.pdf`, true)
        if (returnBlob) return blob; toast({ title: "Credit note PDF downloaded!" })
      }
  ...
  const handleWhatsAppShare = async () => {
    ...
    try {
      ...
      const fileName = `credit-note-${creditNoteNo || "document"}.pdf`
      const blob = await exportNodeToPdf(node, fileName, true)
      const msg = `Hello,\n\nPlease find attached the credit note.\n\nBest regards`
      const shared = await tryNativeShare(blob, fileName, `Credit Note ${creditNoteNo || ""}`, msg)
      if (!shared) {
        const url = await generateShareUrl(invoiceData, "MinimalMonoTemplate", `Credit Note ${creditNoteNo || ""}`)
        if (url) {
          openWhatsApp(`${msg}\n\nView online: ${url}`)
        } else {
  ...
  const handleEmailReceipt = async () => {
    ...
    try {
      ...
      const shared = await tryNativeShare(blob, fileName, `Credit Note ${creditNoteNo || ""}`, body)
      if (!shared) {
        const url = await generateShareUrl(invoiceData, "MinimalMonoTemplate", `Credit Note ${creditNoteNo || ""}`)
        if (url) {
          openEmail("", `Credit Note ${creditNoteNo || ""}`, `${body}\n\nView online: ${url}`)
        } else {
          const dlUrl = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = dlUrl; a.download = fileName; a.click(); URL.revokeObjectURL(dlUrl)
          openEmail("", `Credit Note ${creditNoteNo || ""}`, body)
        }
  ...
  return (
    <>
      ...
      <div id="credit-note-print-root" className="absolute -left-[9999px] -top-[9999px]" aria-hidden="true">
          ...
          <div className="relative w-full h-full flex flex-col max-w-[1200px] mx-auto bg-white/5 dark:bg-black/5 shadow-2xl animate-in slide-in-from-bottom-8 zoom-in-95 duration-500 overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-white/10 bg-white dark:bg-gray-950 sticky top-0 z-10 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <h2 className="text-lg font-display font-semibold hidden sm:block">Credit Note Preview</h2>
              ...
              <div className="flex items-center gap-2">
                <Button size="sm" className="gap-2 bg-gradient-to-r from-amber-500 to-yellow-600 text-white border-0 font-semibold" onClick={() => handleDownloadPDF()} disabled={isGenerating}>
        ...
        <div className="max-w-4xl mx-auto w-full">
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-3 pb-6">
              <Button onClick={togglePreview} className="gap-2 bg-gradient-to-r from-amber-500 to-yellow-600 text-white border-0 font-semibold flex-1"><Eye className="h-4 w-4" /> SHOW PREVIEW</Button>
              ...
              <button onClick={handleEmailReceipt} title="Email Credit Note" className="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-input bg-background shadow-sm transition-colors hover:bg-accent cursor-pointer"><img src="/email.svg" alt="Email" className="h-5 w-5" /></button>
              <ShareButton invoiceData={invoiceData} template="MinimalMonoTemplate" title={`Credit Note ${creditNoteNo || ""}`} />
