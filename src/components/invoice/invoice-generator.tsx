...
import {
  ...
  RotateCcw, Share2, Loader2, LayoutTemplate
} from "lucide-react"
import {
  InvoicePreview,
  StudioTemplate,
...
export function InvoiceGenerator() {
  ...
  const handleEmailInvoice = async () => {
    ...
    } catch { toast({ title: "Failed to share invoice", variant: "destructive" }) }
  ...
  const handleWhatsAppShare = async () => {
    ...
    try {
      ...
      const shared = await tryNativeShare(blob, fileName, `Invoice ${watchedValues.invoiceNumber}`, msg)
      if (!shared) {
        const url = await generateShareUrl(invoiceData, watchedValues.template, `Invoice ${watchedValues.invoiceNumber}`)
        if (url) {
          openWhatsApp(`${msg}\n\nView online: ${url}`)
        } else {
          const dlUrl = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = dlUrl; a.download = fileName; a.click(); URL.revokeObjectURL(dlUrl)
      ...
      <div className="flex flex-col min-h-[calc(100vh-8rem)]">
        ...
        <div className="flex items-center justify-between mb-6 sticky top-0 z-20 bg-gray-50/80 dark:bg-gray-950/80 backdrop-blur-sm -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 py-4 border-b border-border">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" className="gap-1.5 h-8 text-xs flex" onClick={handleSaveInvoice}>
        ...
        <div className="max-w-4xl mx-auto w-full">
          <div className="space-y-6 min-w-0">
            ...
            <div className="flex flex-wrap gap-3 pb-4">
              ...
              <button onClick={handleEmailInvoice} title="Email Invoice" className="h-9 w-9 inline-flex items-center justify-center rounded-lg border border-input bg-background shadow-sm transition-colors hover:bg-accent cursor-pointer">
                <img src="/email.svg" alt="Email" className="h-5 w-5" />
              </button>
              <ShareButton invoiceData={invoiceData} template={watchedValues.template} title={`Invoice ${watchedValues.invoiceNumber}`} />
