...
export function CreditNoteDocument({ invoice, className = "", config }: CreditNoteDocumentProps) {
  const docConfig = config || getDocumentConfig(invoice.documentType || "credit-note")
  const totals = computeInvoiceTotals(invoice)
  const { company, billTo } = invoice

  const format = (n: number, sym?: string) => formatCurrency(n, sym || invoice.currencySymbol || "₹")
  const isGstEnabled = invoice.gstMode !== "none"
        ...
        <div>
          ...
          <section className="my-5 grid grid-cols-2 gap-4">
            ...
            <div className="p-3.5 rounded bg-[#F4F4F4] border border-[#D6D6D6] space-y-1">
              <p className="text-[11px] font-bold uppercase tracking-wider text-[#444444] border-b border-[#E0E0E0] pb-1">
                Credit Adjustment Notice
              </p>
              <p className="text-[13.5px] text-[#444444]">
                This credit note adjusts against the referenced original invoice for returned merchandise, post-sale pricing adjustments, or commercial bill corrections.
              </p>
            </div>
          </section>
            ...
            <table className="w-full text-[14px] border border-[#D6D6D6] rounded overflow-hidden">
              <tbody>
                ...
                {isGstEnabled && totals.totalGst > 0 && (
                  <tr className="border-b border-[#E5E5E5]">
                    <td className="py-2 px-3 text-[#444444] bg-[#FAFAFA] border-r border-[#D6D6D6]">GST Adjustment</td>
                ...
                <tr className="bg-[#F4F4F4] border-t-2 border-[#111111]">
                  <td className="py-3 px-3 font-bold uppercase tracking-wider text-[#111111] text-[13.5px] border-r border-[#D6D6D6]">
                    {docConfig.totalLabel || "Total Credit Amount"}
                  </td>
